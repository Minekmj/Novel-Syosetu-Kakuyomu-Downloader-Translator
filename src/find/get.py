...
class MidnightSearch:
    ...
    # 미드나이트 노벨즈는 장르 구분이 없으므로 빈 딕셔너리로 유지
    GENRES = {}
    ...
    def fetch_search_results(params):
        ...
        # 장르 (GENRES 딕셔너리가 비어있더라도 파라미터가 들어올 경우 대비)
        genre_val = params.get("genre_val")
        if genre_val and int(genre_val) != 0:
            payload["genre"] = int(genre_val)
        ...
        # 최종 갱신일 필터링
        period_key = params.get("last_published")
        days = MidnightSearch.LAST_PUBLISHED_PERIODS.get(
        ...
        try:
            res = http_session.get(
                api_url,
                params=payload,
                headers=MidnightSearch.HEADERS,
                timeout=10,
            )
            res.raise_for_status()
            data = res.json()
